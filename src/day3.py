...
wires_strings = wires_real
wires_text_lst = wires_strings.splitlines()
wire1_txt = wires_text_lst[0].split(",")
wire2_txt = wires_text_lst[1].split(",")


def moves_to_coordinates(moves):
    coord_lst = [(0, 0)]
    steps_dict = {}
    tot_steps =0
    for m, move in enumerate(moves):
        direction = move[0].lower()
        steps = int(move[1:])
        ...
        for i in range(steps):
            ...
            if not new_coords in steps_dict:
                steps_dict[new_coords]=tot_steps
                #always lower if existing
            coord_lst.append(new_coords)
    return coord_lst, steps_dict


def intersection(a, b):
    return set(a).intersection(b)
#     return [value for value in lst1 if value in lst2] #slow

coords1,steps1 = moves_to_coordinates(wire1_txt)
coords2, steps2 = moves_to_coordinates(wire2_txt)

print(len(coords1))
print(len(coords2))

intersecting = intersection(coords1, coords2)
intersecting.remove((0,0))
manhattan_dist = [abs(x[0])+abs(x[1]) for x in intersecting]

print("i",intersecting)

print("manhattan",sorted(manhattan_dist)[1])

a=[steps1[intersect] + steps2[intersect] for intersect in intersecting]

print("Part2", sorted(a)[0])
